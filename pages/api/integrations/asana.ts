  ...
  if (method === 'GET') {
    ...
    if (action === 'connect') {
      if (isConfigured) {
        ...
        const redirectUri = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/integrations/asana?action=callback`;
        ...
        try {
          const tokenResponse = await fetch('https://app.asana.com/-/oauth_token', {
            ...
            body: new URLSearchParams({
              ...
              redirect_uri: `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/integrations/asana?action=callback`,
              code,
            }),
          });

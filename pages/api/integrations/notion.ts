  ...
  if (method === 'GET') {
    ...
    if (action === 'connect') {
      if (isConfigured) {
        ...
        const redirectUri = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/integrations/notion?action=callback`;
        ...
        try {
          const tokenResponse = await fetch('https://api.notion.com/v1/oauth/token', {
            ...
            body: JSON.stringify({
              ...
              code,
              redirect_uri: `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/integrations/notion?action=callback`,
            }),
          });
